...
class Db_model extends CI_Model {
	...
	public function raz_match($id){
		...
		$query = $this->db->query("UPDATE T_MATCH_MAT
									SET MAT_DEBUT='".$tomorrow->demain."', MAT_FIN=null
									WHERE MAT_CODE='".$id."';");
	...
	public function create_match($quiz, $username){
		...
		$query = $this->db->query("INSERT INTO T_MATCH_MAT VALUES
									(NULL, $quiz, $id_usr->USR_ID, NOW(), NULL, '".$code->code."', 1, 0);");
		return ($query);
	}


	//fonction de calcul du score d'un joueur (et mise à jour dans table T_JOUEUR_JOU) après participation à un match
	public function set_score_joueur($score, $pseudo) {
		$query = $this->db->query("CALL set_score('".$score."', '".$pseudo."');");

		return ($query);
	}


	public function count_questions($code){
		$query = $this->db->query("SELECT COUNT(*) as NbQst 
									FROM T_QUESTION_QST
									JOIN T_QUIZ_QUI USING (QUI_ID)
									JOIN T_MATCH_MAT USING (QUI_ID)
									WHERE MAT_CODE='".$code."';");
