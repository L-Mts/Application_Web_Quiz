...
class Formateur extends CI_Controller {
    ...
    public function modifier_mdp() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->db_model->check_mdp($this->session->userdata('username'))) {
    ...
    public function afficher_match($mat_code=FALSE){
        if ($mat_code==FALSE) {
            ...
            if($data['nb_participants']->nb_participants != 0) {
    ...
    public function raz_match() {
        ...
        $this->load->view('templatesback/haut');
        $this->load->view('templatesback/menu_formateur');
        $this->load->view('formateur_gestion_match', $data);
        $this->load->view('templatesback/bas');
    ...
    public function activer_match() {
        $data['message'] = 'activer';
        $code = $this->input->post('code');
        $this->db_model->activate_match($code);
        $this->load->view('templatesback/haut');
        $this->load->view('templatesback/menu_formateur');
        $this->load->view('formateur_gestion_match', $data);
        $this->load->view('templatesback/bas');
    ...
    public function desactiver_match() {
        $data['message'] = 'desactiver';
        $code = $this->input->post('code');
        $this->db_model->deactivate_match($code);
        $this->load->view('templatesback/haut');
        $this->load->view('templatesback/menu_formateur');
        $this->load->view('formateur_gestion_match', $data);
        $this->load->view('templatesback/bas');
    ...
    public function creer_match() {
        ...
        if($this->form_validation->run() == FALSE) {
            ...
            $this->db_model->create_match($quiz, $username);
            $this->load->view('templatesback/haut');
            $this->load->view('templatesback/menu_formateur');
            $this->load->view('formateur_gestion_match', $data);
            $this->load->view('templatesback/bas');
